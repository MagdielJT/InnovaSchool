 ...
 */
define(['N/runtime','N/search','./Model/innovaFamilyBusquedas.js','N/file','N/https'],
    /**
     * @param{runtime} runtime
     */
    (runtime,search,innovaBusquedas,file,https) => {
            ...
            try {
                ...
                return search.create({
                    ...
                    [
                        ["type","anyof","CustInvc"], 
                        "AND", 
                        [["custbody_mx_cfdi_uuid","isnotempty",""],"OR",["custbody_efx_fe_uuid","isnotempty",""]], 
                        "AND", 
                        ["customer.status","anyof","13","16"], 
                        "AND", 
                        ["taxline","is","F"], 
                        "AND", 
                        ...
                        ["mainline","is","T"]
                        // "AND",
                        // ["custbody_efx_alumno","anyof",listaAlumnos],
                    ],
                    columns:
                    [
        ...
        const map = (mapContext) => {
            try {
                let tranId = JSON.parse(mapContext.value).values.tranid.value
                // let saldosAFavor = innovaBusquedas.saldosAFavor(tranId)
                // let facturasAbiertas = innovaBusquedas.facturasAbiertas(tranId)
                // let pagos = innovaBusquedas.pagosAplicados(tranId)
                // let notasCredito = innovaBusquedas.notasDeCredito(tranId)

                log.debug("mapContext: " + mapContext.key, JSON.parse(mapContext.value) ).values
                // let fileObj = file.create({
                //     name: mapContext.key + 'map.json',
                //     fileType: file.Type.JSON,
                //     contents: JSON.stringify(JSON.parse(mapContext.value))
                // });
